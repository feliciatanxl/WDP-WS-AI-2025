...
from models import db, GroupLeader, Customer
from whatsapp.app import app 

def seed_test_user():
    with app.app_context():
        # --- 1. ENSURE A LEADER EXISTS ---
        # A customer must belong to a leader to process orders.
        test_leader = GroupLeader.query.filter_by(name="Test Leader").first()
        if not test_leader:
            test_leader = GroupLeader(
                name="Test Leader",
                phone="6500000000", # Placeholder phone for the leader
                area="Singapore Central"
            )
            db.session.add(test_leader)
            db.session.commit() # Commit to generate the ID for leader_id
        ...
        if not existing_c:
            ...
            db.session.add(new_customer)
            print(f"👤 SUCCESS: Added {your_name} ({your_phone}) as a registered customer.")
        else:
            # Update the existing record in case you changed leaders or names
            existing_c.name = your_name
            existing_c.leader_id = test_leader.id
            print(f"ℹ️ {your_name} already exists. Record updated to ensure links are active.")

        db.session.commit()
        print("✅ Seeding complete. You can now test the WhatsApp bot.")
